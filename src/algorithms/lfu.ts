import type { Snapshot, LFUVariables } from '../types';
import { emptyFrame, cloneFrames } from './utils';

export function runLFU(sequence: number[], frameCount: number): Snapshot[] {
  ...
  for (let step = 0; step < sequence.length; step++) {
    ...
    if (hitIndex !== -1) {
      ...
      if (emptySlot !== -1) {
        ...
        // Reemplazar la de menor frecuencia; si empatan, la menos recientemente usada
        replaceIdx = newFrames.reduce((minIdx, f, i) => {
          const minF = newFrames[minIdx];
          if (f.frequency < minF.frequency) return i;
          if (f.frequency === minF.frequency && f.lastUsed < minF.lastUsed) return i;
          return minIdx;
        }, 0);
      }
...
function buildVars(frames: ReturnType<typeof cloneFrames>): LFUVariables {
  return {
    type: 'LFU',
